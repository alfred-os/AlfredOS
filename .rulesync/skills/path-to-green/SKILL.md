...
- **Type failure** (mypy / pyright): fix the type signature or add a justified `# type: ignore[<code>]  # reason: ...`. **If the line mentions `TaggedContent`, a suppressor is not available — and neither is a file-wide `ruff: noqa` / `flake8: noqa` / `mypy: ignore-errors` anywhere in a module that mentions it** — `scripts/check_tag_t3.py` refuses every one of them there (a required check), so reaching for it trades one red gate for another. Fix the type, or route the construction through `tag_t3_with_nonce()` (#539, ADR-0059).
