...
- **Never write a source probe that can skip-and-pass.** If a step writes `has_*=true`/`false` to `$GITHUB_OUTPUT` and later steps are gated on `if: steps.<id>.outputs.has_* == 'true'`, the negative branch MUST `exit 1`, not emit a `::notice::` and let the job pass. These directories are permanent, so "found nothing" means the PROBE broke — a skip reports success while gating nothing. Pinned by `tests/unit/test_workflow_invariants.py::test_presence_probes_fail_closed`, which derives the probe set from the workflow YAML, so a new workflow is covered automatically.
- **Never probe with `find … | grep -q`** (or any producer piped into an early-exiting reader: `grep -q`, `head -1`, `sed q`, `read`). `grep -q` exits on the first match, the producer takes SIGPIPE, and `set -o pipefail` turns that into a false negative — it is a size race, so it passes in review and fails once the tree grows. Use `[ -n "$(find … -print -quit)" ]`, or a here-string `grep -Eq 'pat' <<<"$var"`. Pinned by `test_no_pipe_into_an_early_exit_reader`. Both rules come from [#514](https://github.com/alfred-os/AlfredOS/issues/514), where six jobs — including the required `tag(T3)` security gate — reported green while executing nothing for about a month.
