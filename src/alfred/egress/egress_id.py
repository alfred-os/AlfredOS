"""Deterministic, injective egress-id + body-hash for the egress ledger (Spec C §5).
...
not double-fire it. The dedup key is the ``egress-id``: a deterministic, injective
function of the committed per-turn anchor ``(adapter_id, inbound_id, session_id)``
and the logical call position (``call_index``) — NEVER completion-order, which
mis-keys a concurrent fan-out.

Injectivity rides an **unambiguous length-prefixed encoding**: each field is
emitted as its UTF-8 byte length (8-byte big-endian) followed by its bytes, so no
separator can be forged across a field boundary — ``(inbound=1, session=23)`` and
``(inbound=12, session=3)`` hash distinctly. The id is the sha256 hexdigest of that
encoding (64 lowercase hex chars — the ledger PK width).
"""
...
def _length_prefixed(*fields: str) -> bytes:
    ...
    over the field sequence, so the sha256 over it is too.
    """
    out = bytearray()
    for field in fields:
...
def compute_egress_id(ctx: TurnEgressContext, *, call_index: int) -> str:
    """The deterministic, injective dedup key for one logical egress call."""
