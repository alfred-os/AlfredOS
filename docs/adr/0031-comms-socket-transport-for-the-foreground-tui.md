...
## Amendment — the turn.* client-state frame family (#593)

`#593` adds a SIBLING frame family to the `link.*` frames above: the same "the carrier relays an opaque state signal, the TUI renders its own localized text" shape, but the OPPOSITE direction. `link.*` originates AT the gateway ("TUI banners" above); `turn.*` originates in the CORE and is merely relayed byte-for-byte by the gateway, unconsumed. `GatewayCoreLink._route_unit` (`src/alfred/gateway/core_link.py`) recognises only a handful of named methods (`daemon.comms.ack`, the two `daemon.lifecycle.*` frames, `core.adapter.spawn_grant`); every other method — `turn.failed` included — falls through to `_payload_relay` untouched. That fallthrough is what lets `turn.failed` ship with ZERO gateway code change: the gateway does not need to know the frame exists.

`turn.failed` (`TURN_FAILED`, `TurnFailureStage`, `TurnFailedNotification`) is defined in `src/alfred/comms_mcp/protocol.py`. Client-side, the TUI plugin's `plugins/alfred_tui/src/alfred_tui/cohost.py` routes it in its own branch, parallel to but not folded into the `_LINK_STATE_METHODS` allowlist that routes `link.*`: unlike a `link.*` frame (method-only), `turn.failed` carries a payload (the closed `stage` Literal) that must be Pydantic-validated at the wire, so it needs its own branch rather than the banner allowlist's method-only check.
