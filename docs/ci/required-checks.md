...
| `Integration` | `.github/workflows/ci.yml` | `integration` | 2026-06-06 | Integration test suite under `tests/integration/` against real testcontainers (Postgres, Redis). Appends coverage data to the artifact `coverage-gates` consumes. PR-S4-6: the job now apt-installs `bubblewrap` + `jq` so the merge-blocking `test_launcher_policy_resolver.py` (skipif-bwrap-missing locally) actually exercises the real bwrap sandbox in CI rather than silently skipping. **PR-S4-9 merge-blocking gates** run inside this job — this repo aggregates `tests/integration/` under one `Integration` check rather than emitting a separately-named GitHub context per test (adding per-test contexts that no job emits would block all PRs, per the path-filter caveat below): `tests/integration/test_discord_addressing_modes.py` (the four addressing modes round-trip) and `tests/integration/test_discord_subpayload_promotion.py` (the nine sub-payload kinds promote to a `ContentHandle` against a real Redis `ContentStore`, orchestrator never seeing raw bytes). **PR-S4-10 merge-blocking gate** joins them under the same aggregate: `tests/integration/test_tui_round_trip.py` (the TUI MCP plugin round-trip — `plugins/alfred_tui` spawned through `bin/alfred-plugin-launcher.sh` for the stdio outbound leg, a keystroke-batch reaching the host `process_inbound_message` with the `IdentityResolver` consulted exactly once and the canonical id never crossing the wire, and the outbound reply rendering to the real `AlfredTuiApp` RichLog). A regression in any of them fails the `Integration` check and blocks merge. |
