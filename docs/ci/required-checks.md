...
| `Smoke (end-to-end)` | `.github/workflows/ci.yml` | `smoke` | 2026-06-06 | End-to-end TUI smoke against an in-process mock provider (and the real provider when a non-placeholder `ALFRED_SMOKE_PROVIDER_KEY` is configured). Pins the TUI → orchestrator → episodic-write → audit-row contract. |
