...
- **Coverage:** ONLY the resolver (`_environment_loader.py`) gets a NEW per-module **`ci.yml`** `coverage report --fail-under=100` gate (NOT `make check` — it runs 0 of 47, #474) — CR4 (fleet review): `settings.py`'s `mode="wrap"` validator is deliberately **NOT** whole-file coverage-gated (it carries one pre-existing uncovered line unrelated to environment resolution; see [#488](https://github.com/alfred-os/AlfredOS/issues/488)), so do not read "the resolver + Settings validator" as two gated targets. `_commands.py`/`_failures.py`/`adapter_child_factory.py`/`manifest_reader.py` are **already** 100%-gated (pre-existing, unrelated to this plan) — every new branch in them must still be covered or those gates red. `environment` is security-load-bearing → run the **full adversarial suite** (Task 9).
