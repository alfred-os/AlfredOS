...
5. **devops-1 HIGH (bwrap version pin)**: PR-S4-0b's Dockerfile MUST pin `bubblewrap=0.8.0-1` (Debian Bookworm). The fd-inheritance flag is `--keep-fd FD` ("Do not close fd FD"), present since bubblewrap **0.5.0** and still current in **0.9.0** (ADR-0015). `--sync-fd` is bwrap's unrelated INTERNAL sync-protocol fd — NOT a data channel — and must NOT be used for key delivery; issue #218 (claiming a `--keep-fd`→`--sync-fd` rename) is a misdiagnosis, corrected by ADR-0015. The launcher/translator emit `--keep-fd`, so the pinned image and any CI apt-installed bwrap MUST advertise `--keep-fd`. PR-S4-1's daemon-boot probe adds `bwrap_version_check` (#228): parse `bwrap --version`, refuse boot if the installed bwrap does not accept `--keep-fd`. New i18n key `daemon.boot.bwrap_version_too_old`. New corpus entry `sbx-2026-008-bwrap_version_drift` simulates a drifted bwrap and asserts boot refusal. **The CI Integration job (PR-S4-6) asserts `bwrap --help` advertises `--keep-fd` so a stripped/forked bwrap fails LOUDLY rather than silently mis-running the resolver test.**
...
> **Flag reconciliation (corrects #218 / ADR-0015).** The correct fd-inheritance flag is **`--keep-fd 3`** ("Do not close fd 3"), present since bubblewrap **0.5.0** and still current in **0.9.0**. `--sync-fd` is a DIFFERENT flag — bwrap's internal sync-protocol fd — which bwrap consumes/closes and does NOT pass through to the child; using `--sync-fd 3` makes the sandboxed plugin's `os.read(3, ...)` fail with `Bad file descriptor`. Issue #218 (claiming a `--keep-fd`→`--sync-fd` rename in 0.8.0) is a misdiagnosis: both flags exist and are distinct. This plan and the translator (`policy_to_bwrap_flags`) emit `--keep-fd`. The policy field name `keep_fds` matches the emitted flag. PR-S4-7 (macOS/Windows policy bytes) reads this plan as its contract and MUST emit `--keep-fd`. ADR-0015 owns this invariant; the #228 boot-probe enforces the bwrap version floor.
...
| bwrap `--keep-fd` flag | Per bwrap upstream docs + empirical CI proof (bwrap 0.9.0). `--keep-fd FD` is the fd-inheritance flag (0.5.0+); `--sync-fd` is bwrap's unrelated internal sync fd — #218's claimed rename is a misdiagnosis (ADR-0015) | **verified — emit `--keep-fd`; DO NOT use `--sync-fd` (child sees fd 3 as Bad file descriptor) nor a `/dev/fd/3` mount** |
