...
5. **devops-1 HIGH (bwrap version floor)**: PR-S4-0b's Dockerfile pins `bubblewrap=0.8.0-1` (Debian Bookworm). The translator emits NO fd flag (see the fd-3 contract callout) — fd 3 inherits by default in 0.8.0 AND 0.9.0, so there is no version-bound flag name to guard. PR-S4-1's daemon-boot probe (#228) parses `bwrap --version` and refuses boot below the supported floor (general bwrap-presence/version check, not a flag check). New i18n key `daemon.boot.bwrap_version_too_old`. The `sbx-2026-008` payload covers fd-3 partial-write delivery (the version-drift boot probe is tracked separately in #228; no dedicated corpus payload yet). The earlier `--sync-fd` advertise-check has been REMOVED from CI (it was checking the wrong thing).
...
3. `bin/alfred-plugin-launcher.sh` runs the pre-launcher Python helper to read the `sandbox` block and branches:
   - `kind: full` → resolve OS-specific `policy_refs.{linux,macos,windows}`; refuse if missing/unreadable/OS-mismatch; `exec bwrap ... -- ${PLUGIN_BINARY}` (no fd flag) on Linux. macOS+Windows bytes ship in PR-S4-7; PR-S4-6 ensures the *resolver path* compiles for all three OSes.
...
5. Supervisor (Python) ships fd-3 provider key delivery: `SecretBroker.get("quarantined.provider_key")` → 4-byte big-endian length prefix + key bytes → fd 3 → close write end → `gc.collect()`. bwrap inherits fd 3 by default (no flag); bash never touches the bytes.
...
```
        ...
        │                On Linux: exec bwrap (no fd flag)              │
        │                  $(cat "$POLICY_REF" |                     │
        │                   python3 -m                               │
        │                   alfred.plugins.manifest_reader           │
...
> **fd-3 contract (#218 / ADR-0015) — supersedes any `--sync-fd`/`--keep-fd` wording elsewhere in this plan.** The translator emits **NO fd flag**: bwrap inherits open, non-CLOEXEC fds (fd 3 = the provider-key channel) into the sandboxed child BY DEFAULT. Verified empirically in a docker bwrap repro against 0.8.0 (the pinned image) and 0.9.0: no flag → key delivered. `--sync-fd FD` is bwrap's INTERNAL sync fd ("Keep this fd open while sandbox is running" keeps it open in bwrap's monitor) — pointing it at fd 3 CONSUMES the descriptor, severing delivery (the plugin's `os.read(3)` raises EBADF). There is no `--keep-fd` in 0.8.0/0.9.0; the "0.9.0+ rename" belief was a misconception. The logical field name `keep_fds` is a validated declaration only (arch-2). **Separate, load-bearing:** the spawning parent MUST place the pipe read end ON fd 3 (`os.dup2(read_fd, 3)` in the parent + `pass_fds=(3,)`; a `preexec_fn` dup2 does NOT work — close_fds runs after preexec and closes the dup'd fd 3). PR-S4-7 reads this plan as its contract and MUST emit no fd flag + use this spawn pattern. ADR-0015 owns this invariant.
...
| `bin/alfred-plugin-launcher.sh` | Modify | Add: `Settings.environment` read; dev-escape-hatch refuse-in-production with stderr message; manifest sandbox block read via `manifest_reader.py`; `kind` branching (`full` / `none` / `stub`); `bwrap` invocation (no fd flag — fd 3 inherits) on Linux for `kind: full`. PRESERVES Slice-3 invariants: charset-validate PLUGIN_ID, help flag, runuser UID-drop on Linux for `kind: none`, fail-closed semantics, structured audit JSON on stderr |
...
| `tests/unit/launcher/test_sandbox_kind_branching.py` | **Create** | bash-fixture-harness test: `kind: full` invokes bwrap (no fd flag; fd 3 inherits by default); `kind: none` falls through to Slice-3 runuser path; `kind: stub` refuses in prod, emits stub-used row in dev |
...
| `tests/integration/test_launcher_policy_resolver.py` | **Create** | Index §4 merge-blocking integration test. End-to-end: real bash launcher process; real manifest_reader subprocess; real fixture policy file; assert bwrap is invoked with no fd flag (fd 3 inherits) and the fixture's policy flags. Skipped if `bwrap` not on PATH (CI image has it after PR-S4-0b) |
...
Fd 3 is the documented out-of-band channel for the quarantined-LLM provider key. PR-S4-6 establishes that fd 3 is **always passed through** by the launcher whether or not the plugin uses it (the cost of inheriting fd 3 when nothing is written to it is zero — the inherited fd is just unused). PR-S4-9 (Discord) and PR-S4-10 (TUI) plugins do NOT consume fd 3 but the launcher still inherits it for forward-compat.
...
- [ ] **Task G.5 — Failing test: sandbox-kind branching.**
  ...
  - `kind: full` → bwrap invocation observed (use a `BWRAP=/path/to/echo-bwrap-args` env var override the launcher honours; ship `tests/fixtures/echo-bwrap.sh` that prints its args + exits 0; assert NO `--sync-fd`/`--keep-fd` appears in the printed args (fd 3 inherits)).
...
- [ ] **Task I.4 — Make I.3 pass.**
  ...
  Verify the launcher's bash code never references fd 3 in any `read` call. bwrap inherits fd 3 by default (no flag); bash itself never reads it. The test should pass on first try — the launcher was designed for this.
...
- [ ] **Task K.2 — Update `docs/glossary.md`.**
  ...
  - **fd-3 inheritance** — the kernel-managed pattern by which the Supervisor delivers a secret to a sandboxed plugin without bash buffering the bytes. bwrap inherits fd 3 by default (no fd flag) — the documented mechanism.
...
| bwrap absent/below the version floor | PR-S4-0b's apt install gives bwrap ≥ 0.6 on alfred-core. CI image gates on bwrap version. PR description states the floor | LOW |
