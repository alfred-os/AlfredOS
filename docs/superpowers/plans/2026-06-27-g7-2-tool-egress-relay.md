...
> **STATUS (2026-06-27): every R2 item below is now FOLDED into the executable Part B/C/D task bodies** (commit "fold G7-2 findings into executable B/C/D tasks + C1=(B) split"). This block is retained as the review record; the imperative voice describes what the fold did, not outstanding work. The C1 scope was resolved to option **(B)** — the live web.fetch re-home is **Part D / G7-2.5**, and the stale "Task C3 re-point" no longer exists (Part C ends at the synthetic-driver-proven relay).

- **R2-Critical (ARCH-1) `[FOLDED]` — C1 not folded into the executable tasks.** The C1 reconciliation was recorded but the (then) Task C3 still described the impossible "re-point"; G7-2c-2's end-to-end exit was unachievable until the C1 rework landed in the tasks. **Folded:** C1=(B) split — the re-point became Part D / G7-2.5; Part C now ends at the synthetic-driver-proven relay.
...
- Produces:
  ...
  - `EgressResponse` — frozen Pydantic: `status: int`, `headers: Mapping[str, str]`, `body: str`. `frozen=True, extra="forbid"`. **The response body is raw T3 and may be arbitrary bytes** (web responses are not necessarily UTF-8). Pin the wire representation in B1 so it round-trips losslessly across the JSON frame: either a `bytes` field with explicit base64 on the frame, or a `body_b64: str` field, or an explicit lossless decode (`latin-1`) — NOT a naive `bytes.decode("utf-8")` that raises/corrupts on binary. C2 mints the `ContentHandle` from the exact bytes, so the chosen representation must be byte-exact. (B4/C1 must agree on the same representation — it's the wire contract.)
  - `_RawToolRequest` (H4 — define it here; consumed by C1/C2) — frozen Pydantic, the in-core pre-redaction tool request: `method: str`, `url: str`, `headers: Mapping[str, str]`, `body: str = ""` (the **raw** body the core will redact; defaults to empty for the live GET-only consumer, which sends no body), `idempotent: bool = False` (the manifest-declared idempotency flag — gates the H3 in-doubt auto-refire; default `False` = in-doubt refuses). `frozen=True, extra="forbid"`. Modelled as empty/optional-body DiD infra per ARCH-3 — only the synthetic body-sending driver populates `body`.
