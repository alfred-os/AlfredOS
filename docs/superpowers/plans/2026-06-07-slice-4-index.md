...
| PR-S4-6 | sandbox-launcher | `bin/alfred-plugin-launcher.sh` policy-resolving extension (bash, not Python — sec-004 honest); mandatory `Settings.environment`; dev-escape-hatch operator-visible stderr; quarantined-LLM manifest update; `bwrap --sync-fd 3` fd-3 inheritance pattern. |
