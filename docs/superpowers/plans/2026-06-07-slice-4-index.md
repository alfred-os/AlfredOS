...
6. **Sandbox launcher foundations** (ADR-0015) — `bin/alfred-plugin-launcher.sh` (bash) extended with policy-resolving behaviour via a pre-launcher Python manifest reader. `Settings.environment` mandatory (no fallback). Dev escape hatch refuses in production with operator-visible stderr message. Provider key handled via fd-3 inheritance pattern (bwrap inherits fd 3 by default — NO fd flag; `--sync-fd` would consume fd 3, verified 0.8.0/0.9.0, #218 / ADR-0015). The key-handling control is the single atomic `os.writev` of `[len-prefix | key]` to the fd-3 pipe followed by explicit post-write zeroization of the byte buffer (`fd3_key_delivery`); the residual `str`-interning window between broker fetch and write is the acknowledged limitation Slice-5 `SecretBroker.get_bytes` closes, with `gc.collect()` a secondary mitigation, not the control. PR-S4-6.
...
| PR-S4-6 | sandbox-launcher | `bin/alfred-plugin-launcher.sh` policy-resolving extension (bash, not Python — sec-004 honest); mandatory `Settings.environment`; dev-escape-hatch operator-visible stderr; quarantined-LLM manifest update; fd-3 inheritance pattern (bwrap inherits fd 3 by default — NO fd flag; #218 / ADR-0015). |
