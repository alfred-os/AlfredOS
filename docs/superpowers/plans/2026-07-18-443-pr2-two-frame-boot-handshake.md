...
**Goal:** Move the quarantined-LLM child's launcher-refusal detection from first-extraction to boot, by having `spawn_quarantine_child_io` read a two-frame boot handshake (`hello` before `_build_provider`, `ready` before the request loop) from the child INSIDE the spawn — so a **handshake-observable** launcher refusal (the launcher exits pre-`exec`, the `hello` read hits a zero-byte EOF) refuses boot with an attributed audit row instead of surfacing late. This guarantee is scoped to the handshake-observable arm: the **fast-refusal EPIPE path** (ADR-0051 §8.4 / spec §11.4) is a named accepted residual — the launcher's fast exit closes the last fd-3 read end before the `writev`, so `deliver_provider_key_via_fd3` raises `ProviderKeyDeliveryError` and boot refuses fail-closed *before* `_SubprocessChildIO` (and its handshake) is ever constructed, producing **no attributed row**. Closing that arm needs option (C) or a conscious accept, not this handshake.
...
YAML `threat`: "A genuine (slow) **handshake-observable** launcher sandbox refusal must refuse BOOT with exactly one attributed `sandbox_refused` row AND an actually-dispatched `fail_closed` T0 hookpoint — not surface late at first extraction." Pin a **slow** reason (`sandbox_block_missing`) in the payload — a fast reason (`plugin_id_charset_invalid`) EPIPEs before the child is constructed (§8.4 / spec §11.4: `ProviderKeyDeliveryError` fires before `_SubprocessChildIO` exists, so boot refuses fail-closed but records NO row), records nothing, and would make the oracle nondeterministic. The "exactly one attributed row" guarantee is therefore scoped to the handshake-observable arm; the fast-EPIPE arm is the named accepted residual, out of this oracle's scope.
