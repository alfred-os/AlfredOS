...
- Three of the four #347 blockers (live-relay `TimeoutError`/`in_doubt` cross-layer audit test, per-user resource bound, broker auth). These are properties of the **web.fetch egress live path**, whose first live exercise is PR4. (The fourth #347 item — `User.language` sourcing — is NOT deferred: Task 3 threads the real `language=user.language` through `dispatch_tool` in PR3, per FIX-9.)
...
## Plan-review fixes (FOLD THESE FIRST — they OVERRIDE the task bodies)

A 5-specialist `/review-plan` fleet (architect, cross-cutting reviewer, test-, security-, core-engineer) ran against rev.1. **No Criticals.** The direction, the Option-A scope call, the 3-grant seed + axis-faithful assertion, and the escalation-audit-durability claim were all independently endorsed. Rev.2 folds the corroborated defects below. Each implementer pastes the FIX items applicable to its task; where a FIX conflicts with a task-body code block, **the FIX wins**. All claims verified against the merged tree.

- **FIX-1 [High, corroborated ×4 — arch-001/rev-001/sec-004/core-005]** — `result="max_iterations_reached"` is NOT in the `ck_audit_log_result` CHECK domain (`memory/models.py:135` = `success, budget_blocked, budget_overrun, provider_failed, unknown_budget_user, cancelled, refused, quarantined, rate_limited, fault, dispatch_failed, …`). Runtime `CheckViolation` at INSERT + the static AST guard fails. **Use `result="refused"`** (in-domain) for the max-iterations exit; the semantic lives in the `phase`/`exit_reason` (FIX-6). **No migration.**
- **FIX-2 [High, corroborated ×3 — arch-002/rev-003/core-004]** — remove the OUTER Decide-phase budget pre-check (`core.py` ~693-739) AND the outer `request = CompletionRequest(messages=messages)` (~691). The loop's iteration-0 pre-check is the ONLY pre-check — otherwise `estimate_for`/`would_exceed` fire twice and break `test_core.py:275-276` (`call_count == 1`). KEEP the Orient `messages` build (689-690).
- **FIX-3 [High, corroborated ×2 — test-001/core-001]** — AUDIT CARDINALITY (the "byte-for-byte" fix). Emit a `provider_call:{iteration}` row ONLY for a **non-terminal** (tool_use) completion that continues the loop. The **terminal** completion (`stop_reason != "tool_use"`) is audited SOLELY by the existing single `"completed"` row. This keeps the no-tools happy path at exactly `audit.append.await_count == 1` + `working.append.await_count == 2` (verified against `test_core.py:283/238`), so ONLY the ~2 provider-**failure** phase strings (`provider_call` → `provider_call:0`) need updating. Cost accounting via a `pending_completion_cost` tracker: each completion sets `pending = response.cost_usd`; a non-terminal completion's `provider_call` row consumes it (`pending = 0.0`); the `completed` row's `cost_actual_usd = pending` (the terminal completion's cost on a normal exit; `0.0` on a refusal exit whose last completion was already `provider_call`-logged). Sum across rows = turn total → NO double-count (resolves core-002).
- **FIX-4 [High, corroborated ×2 — rev-002/core-003]** — in `core.py` use `from alfred.orchestrator import loop_constants` and reference `loop_constants.MAX_TOOL_ITERATIONS` / `.MAX_TOOL_CALLS_PER_ITERATION` / `.TOOL_RESULT_MAX_CHARS` at the USE sites (NOT `from ...loop_constants import MAX_...`). A name-binding import defeats the tests' `monkeypatch.setattr(loop_constants, ...)`, failing `test_fanout_over_cap_refuses` + `test_max_iterations_reached`.
- **FIX-5 [High, corroborated ×2 — sec-001/test-002]** — add escalation-propagation unit tests (Task 3): a fake `dispatch_tool` raising each of `InboundCanaryTripped` / `OutboundCanaryTripped` / the downgrade-clearance `AlfredError` → assert it propagates UNCAUGHT out of `_handle_turn` (spec §9 canary→HALT, HARD rule #7). Cheap + security-critical — do NOT defer to PR4.
- **FIX-6 [Medium — core-006/core-007]** — TERMINAL RESULT + AST guard. (a) Track `final_result_token` set at each exit — normal → `charge_result` (`success`/`budget_overrun`); fan-out → `refused`; max-iterations → `refused`; mid-turn budget → `budget_blocked`. The `completed` row uses `result=final_result_token` + a `subject["exit_reason"]` field, so it never contradicts the exit (core-007). Fold the fan-out / max-iterations / mid-turn-budget exits INTO the `completed` row (no separate audit row) EXCEPT the iteration-0 pre-check, which keeps its `budget_pre_check` row + `raise` unchanged. (b) Update `tests/unit/audit/test_audit_log_result_domain_closed.py::test_dynamic_result_sites_are_documented` — register `core.py`'s new `provider_call:{iteration}` dynamic `result=` site + the moved `completed`-row site (rationale + forwarded in-domain values) and refresh the pinned line numbers. Run FULL `tests/unit` to confirm.
- **FIX-7 [Medium, verified — sec-003]** — Task 4 must fix `tests/unit/security/capability_gate/test_bootstrap_grants.py:60` `(row,) = FIRST_PARTY_SYSTEM_GRANTS` (single-element unpack → `ValueError` at 4 grants). Select the DLP row explicitly: `dlp_row = next(g for g in FIRST_PARTY_SYSTEM_GRANTS if g.plugin_id == "alfred.security._extract_dlp_subscriber")`.
- **FIX-8 [Medium — rev-004]** — Task 4 add a drift-guard test pinning the 3 new grants to the runtime query coordinates (mirror `test_seeded_plugin_id_matches_dlp_subscriber_module`): assert each grant's `(plugin_id, hookpoint, content_tier, subscriber_tier)` matches the constants `dispatch_tool` (`TOOL_DISPATCH_PLUGIN_ID`/`TOOL_DISPATCH_HOOKPOINT`), `quarantined_to_structured` (`alfred.quarantined-llm`/`quarantine.dereference`/`T3`), and `downgrade_to_orchestrator` (`t3.downgrade_to_orchestrator`/`t3.downgrade_to_orchestrator`/`T3`) query with.
- **FIX-9 [Medium — arch-003]** — remove "User.language sourcing" from the PR4 deferral list below. Task 3 threads `language=user.language` (the real source PR2's `dispatch_tool` docstring says is "wired in PR3"), satisfying that #347 item IN PR3.
- **FIX-10 [Medium — test/rev]** — Task 3 add a unit test asserting `dispatch_tool` received the correct kwargs (capture `ctx`/`registry`/`gate`/`dlp`/`audit`/`user_id`/`correlation_id`/`language`); the fake must not blindly discard `**kw`.
- **FIX-11 [Medium — sec-002]** — Task 5's HARD-#5 assertion must be NON-vacuous: the loopback upstream MUST serve a body containing a known marker (`raw-upstream-secret`), and the test asserts BOTH that the marker is ABSENT from every fed-back tool_result AND that the extracted `WebFetchExtraction {text,intent}` shape IS present.
- **FIX-12 [Medium — test/rev]** — Task 5 does NOT reuse a fictional `assembly_env` fixture (`test_tool_assembly.py` is a monolithic inline body). Either refactor `test_tool_assembly.py` to extract a shared fixture/helper (preferred, DRY) OR build Task 5's own harness composing `build_tool_registry` + `_assembly_gate` + a loopback upstream. State which.
- **FIX-13 [Medium — sec-005/test/arch-005]** — Task 4 add a direct test of `_first_party_grant_live` exercising BOTH the subscriber-axis and content-axis branches (not only the `GatePolicy`-level test), and state the 100% line+branch coverage target on the touched `security/` files.
- **FIX-14 [Low — i18n dependency-order]** — add ALL FOUR loop-message i18n keys (`too_many_tool_calls`, `max_iterations_reached`, `budget_exhausted_mid_turn`, `budget_overrun_mid_turn`) in the EARLIEST task that calls `t()` on them (Task 2 — its mid-turn arms use two of them), run `pybabel` there. They stay the user-facing `final_content`, so all four remain needed under FIX-6.
- **FIX-15 [Low, human-judgment — rev/core-002]** — `episodic.record` logs the FINAL completion's cost; the `completed` audit row logs the turn total (`per_turn_spent_usd`). For a multi-completion turn they differ. DECISION (documented, surfaced to the user): episodic = the answer's cost attribution; audit = the turn's total spend — leave the divergence and document it in the persist-step comment. Reopen if the user wants episodic to carry the turn total.
- **FIX-16 [Low — rev]** — in the Task-3 unit tests, note that `gate=MagicMock()` is acceptable ONLY because those tests fake `dispatch_tool` entirely (the gate is never consulted); the REAL gate is exercised in Task 5 (HARD rule #2 — no permissive gate shim on a path that actually hits the gate).
