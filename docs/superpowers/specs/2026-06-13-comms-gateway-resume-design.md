...
> **Status (2026-06-18) — #237 graduation criterion #7 PROVEN.** G4 (the `ReplayBuffer` + reconnect/replay engine) shipped component-complete but was **production-unwired** — `GatewayProcess` built the `GatewayCoreLink` with `replay_buffer=None`, so the always-up gateway still loud-dropped un-acked input on a restart. **G5 first ACTIVATES the resume** (`GatewayProcess` now injects a production-capped `ReplayBuffer` + the determinism seams into the core-link), then the **release-blocking deterministic integration test** `tests/integration/test_gateway_restart_survival.py` proves the assembled chat→gateway→core stack over a REAL `GatewayClientListener` socket: a core restart paints `link.reconnecting → link.restored`, `alfred chat` survives, and un-acked operator input is replayed with fresh seqs in FIFO order (with an already-acked control proving non-vacuity). The live PTY smoke is the nightly complement. The `alfred chat`→gateway re-point + banner rendering were already shipped (PR-S4-237-2); G5 = production activation + the end-to-end proof.
