...
| Quarantined dereference denied | `AlfredError` (`security.quarantine.dereference_denied`) | `src/alfred/security/quarantine.py` | gate-side `security.capability_gate.*` row (per spec §8.2) | hookpoint not granted for the quarantined plugin | queue `alfred plugin grant <id> user-plugin …`; wait for reviewer approval |
