...
| Inbound egress-response canary reflected in a hostile upstream body; the live web.fetch inbound-canary scan is unwired (factory `canary=None`) — a machine-visible #339 merge-blocker residual (xfail until a core-side token source lands) | PR #333 / issue #333 (`de-2026-012` `de_egress_inbound_canary_unwired.yaml` + `test_de_egress_inbound_canary_unwired.py`) |
