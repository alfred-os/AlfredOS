...
| Unbound base dispatch (`BaseModel.copy`/`model_copy` on an instance) and raw state writes (`object.__setattr__`, `__setstate__`) — **still admitted, undefended at both layers**. `BaseModel.model_construct.__func__` was in this row and is now CLOSED: pydantic invokes `cls.model_post_init` from inside the base implementation | #518 follow-on (`tl-2026-013` `tl_base_dispatch_and_raw_state_write.yaml`) — `out_of_scope=true` for runtime by construction; authoring-layer detection is the #518 detector follow-up |
