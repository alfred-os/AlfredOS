...
| `TaggedContent` subclass shadowing `_validate_tier` / `_assert_tier_admissible` — pydantic rebinds validator targets by NAME off the subclass MRO, so any subclass is a lever on the guard | #518 follow-on (`tl-2026-012` `tl_tagged_content_subclass_validator_shadow.yaml` + `test_tier_laundering_copy_seams.py`) — `__init_subclass__` module check + name-mangled model validator + module-level guard dispatch |
| Unbound base dispatch (`BaseModel.copy(obj, …)`, `BaseModel.model_construct.__func__`) and raw state writes (`object.__setattr__`, `__setstate__`) — **still admitted; undefended at BOTH layers today** | #518 follow-on (`tl-2026-013` `tl_base_dispatch_and_raw_state_write.yaml`) — `out_of_scope=true` for runtime by construction; authoring-layer detection is the #518 detector follow-up |
