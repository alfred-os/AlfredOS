...
async def test_zero_subscriber_row_byte_identical(
    ...
    fresh_reg = HookRegistry(gate=make_permissive_fixture_gate())
...
async def test_system_tier_redactor_scrubs_content(
    ...
    * **System tier.** ``make_permissive_fixture_gate(allow_system=True)`` is constructed
    ...
    * **i18n rule #3.** ``language="ja-JP"`` is non-default; the
    ...
    """
    # Install a registry with an EXPLICIT ``make_permissive_fixture_gate(allow_system=True)`` —
    ...
    try:
        ...
        # Register at system tier. ``make_permissive_fixture_gate(allow_system=True)`` is the
