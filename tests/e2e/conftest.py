...
Owns an ISOLATED docker compose project (per-run-unique name + temp --env-file) so a local
run never touches an operator's stack. Fails LOUD (raises) if Docker is absent — never skips.
"""
...
# Single source of truth: the baseline services brought up + asserted healthy are exactly
...
def boot_stack(tmp_path_factory: pytest.TempPathFactory) -> Iterator[BootStack]:
    ...
    try:
        try:
            ...
            # Surface the captured output: CalledProcessError.__str__ omits stdout/stderr and the
            # output was captured (not inherited), so a build/up failure — the likeliest first-run
            # failure — would otherwise leave BOTH the exception and e2e-stack.log empty of the
            # cause (review: devex/error). Re-raise the scrubbed tail so the diagnosis survives.
            tail = _env.scrub_env_secrets((exc.stderr or exc.stdout or "")[-2000:], env_file)
            raise RuntimeError(
                f"e2e stack build/up failed (exit {exc.returncode}):\n{tail}"
            ) from exc
        ...
        try:
            ...
            # Best-effort: a hung/failed `logs` must not skip teardown, but leave a breadcrumb so a
            # missing e2e-stack.log has a traceable reason (review: error-reviewer).
            print(f"::warning::e2e log capture failed: {exc!r}", file=sys.stderr)
