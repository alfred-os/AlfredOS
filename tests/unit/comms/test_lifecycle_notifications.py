...
@pytest.mark.parametrize(
    ...
    [
        ...
        "g" * 32,  # non-hex char
        "a" * 31 + "-",  # right length, non-hex
    ],
)
def test_ready_rejects_malformed_epoch(bad: str) -> None:
    # The epoch wire shape is pinned to uuid4().hex (32 lowercase hex chars), so
    # a wrong-length / wrong-charset token fails validation loudly.
    with pytest.raises(ValidationError):
        ReadyNotification(epoch=bad)
